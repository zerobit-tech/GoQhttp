...
func (pgm *Pgm) ToJson() map[string]any {
	outMap := make(map[string]any)
	if strings.TrimSpace(pgm.Success) != "" {
		outMap["**Success"] = pgm.Success
	}
...
type Data struct {
	...
	Var   string `xml:"var,attr"`
	TextX string `xml:",chardata"`
}

func (data *Data) getValueByType() any {
	return toGoVal(data.Type, data.TextX)
}
func (data *Data) ToJson(outMap map[string]any) {
	...
	if found {
		if typeutils.IsList(val) {
			valList, ok := val.([]any)
			if ok {
				valList = append(valList, data.getValueByType())
			}
			outMap[key] = valList

		} else {
			valList := []any{val, data.getValueByType()}
			outMap[key] = valList

		}
	} else {
		outMap[key] = data.getValueByType()
...
func ProcessSucessXML(xmlData string) (map[string]any, error) {
	...
	return xmlService.Pgm.ToJson(), nil
